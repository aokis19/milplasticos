(function () {
  ...
  function calcularResumoPeriodo(periodoIdParam, excluirSetores = null) {
    ...
    sets.forEach(s => {
      if (!excluir.has(s.id)) {
        const { totalCusto, totalKg } = calcularCustosSetor(s.id);
        custoTotalGeral += totalCusto;
        producaoTotalGeral += totalKg;
      }
    });

    const setoresFinais = sets.filter(s => s.produtoFinal && !excluir.has(s.id));
  ...
  function renderizarSetores() {
    ...
    container.innerHTML = `
      <div class="resumo-consolidado" style="background:linear-gradient(135deg,#2d3748 0%,#1a202c 100%);">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:0.5rem;">
          <h3 style="color:#63b3ed;margin:0;"><i class="fas fa-chart-pie"></i> Fechamento: ${getNomeMes(periodoAtual.mes)}/${periodoAtual.ano}</h3>
