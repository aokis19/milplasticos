// =============================================
// FIREBASE-SYNC.JS - Versão 3.1 Corrigida
// Sincronização Firebase ↔ LocalStorage (Cache)
// CORREÇÃO: Verificação de tamanho antes de salvar no localStorage
// =============================================
...
(function() {
    ...
    const CONFIG = {
        ...
        MAX_BACKUPS: 5,
        SYNC_INTERVAL: 30000,
        MAX_LOCAL_STORAGE_MB: 2, // Limite máximo por módulo
    };
    ...
    // Campos pesados que devem ser removidos do localStorage
    ...
    // Remove campos pesados para economizar espaço
    ...
    // Salva no localStorage com verificação de tamanho
    function safeSetItem(key, data) {
        try {
            ...
            if (sizeMB > CONFIG.MAX_LOCAL_STORAGE_MB) {
                ...
                const dataLimpa = Array.isArray(data) 
                    ? data.map(removerCamposPesados) 
                    : removerCamposPesados(data);
            ...
            console.warn(`  ⚠️ ${key}: Erro ao salvar - ${e.message}`);
    ...
    // ============ INICIALIZAÇÃO ============
    function init() {
        console.log('🚀 Inicializando Sync System v3.1 - Multi-Módulos (Corrigido)');
        ...
        console.log('✅ Sync System v3.1 inicializado');
    ...
    async function baixarDadosFirebase() {
        ...
        try {
            ...
            motoristasSnap.forEach(doc => newData.motoristas.push({ firebaseId: doc.id, ...doc.data() }));
            ...
            pagSnap.forEach(doc => newData.pagamentos.push({ firebaseId: doc.id, ...doc.data() }));
            
            const kmSnap = await db.collection('registrosKM').orderBy('data', 'desc').limit(500).get();
            kmSnap.forEach(doc => newData.registrosKM.push({ firebaseId: doc.id, ...doc.data() }));
    ...
    async function salvarNoFirebase() {
        if (!db || !isOnline) { adicionarFilaSync(); return false; }
    ...
    async function processarFilaSync() {
        ...
        for (let i = syncQueue.length - 1; i >= 0; i--) {
            ...
            if (op.tentativas > 10) { syncQueue.splice(i, 1); continue; }
            op.tentativas++;
            const backup = JSON.parse(JSON.stringify(appData));
            try {
                appData = op.dados;
                if (await salvarNoFirebase()) syncQueue.splice(i, 1);
                else appData = backup;
            } catch (e) { appData = backup; }
        }
        safeSetItem(CONFIG.SYNC_QUEUE_KEY, syncQueue);
        isSyncing = false;
    }
    ...
    // =============================================
    // 🔧 FUNÇÃO CORRIGIDA - sincronizarTodosModulos
    // =============================================
        ...
        for (const [localKey, collection] of Object.entries(MODULE_MAPPINGS)) {
            try {
                ...
                if (!snapshot.empty) {
                    ...
                    if (data.length > 0) {
                        // ✅ USA safeSetItem com verificação de tamanho
                        ...
                        if (result.success && !result.reduced) {
                            ...
                            console.log(`  ⚠️ ${collection}: ${data.length} itens - apenas Firebase (muito grande para cache local)`);
                        }
                    }
                } else {
    ...
    async function addModuleItem(collectionName, item) {
        if (!db || !isOnline) return null;
        try {
            const docData = { ...item, _createdAt: firebase.firestore.FieldValue.serverTimestamp(), _modifiedAt: firebase.firestore.FieldValue.serverTimestamp(), _syncedAt: new Date().toISOString() };
        ...
        for (const [localKey, collection] of Object.entries(MODULE_MAPPINGS)) {
            ...
            if (localData) {
                try {
                    ...
                    resultados.push(`⚠️ ${localKey}: erro`);
                }
            }
        }
    ...
    window.SyncSystem = {
        ...
        updateData: async function(newData, quem = 'usuario') {
            appData = { ...appData, ...newData, _metadata: { ...appData._metadata, version: (appData._metadata.version || 0) + 1, lastModified: new Date().toISOString(), modifiedBy: quem } };
            dataVersion = appData._metadata.version;
            salvarLocalStorage();
        ...
        forceSync: async () => { await processarFilaSync(); await sincronizarTodosModulos(); },
        
        getStatus: () => ({
            online: isOnline, syncing: isSyncing, version: dataVersion,
            queueSize: syncQueue.length, lastSync: appData._metadata.lastSync,
            lastModified: appData._metadata.lastModified, firebaseConnected: !!db
        }),
        ...
        restoreBackup: function(key) {
            ...
            if (str) {
                try { appData = JSON.parse(str); dataVersion = appData._metadata.version || 0; salvarLocalStorage(); return true; }
        ...
        listBackups: () => {
            ...
            for (let i = 0; i < localStorage.length; i++) {
                ...
                if (k && k.startsWith(CONFIG.BACKUP_KEY)) {
                    try { backups.push({ key: k, date: k.replace(CONFIG.BACKUP_KEY, ''), version: JSON.parse(localStorage.getItem(k))?._metadata?.version || 0 }); } catch (e) {}
                }
            }
        ...
        importData: async (jsonData) => {
            try { return await this.updateData(typeof jsonData === 'string' ? JSON.parse(jsonData) : jsonData, 'importacao'); }
            catch (e) { return false; }
        },

        saveModuleData, loadModuleData, deleteModuleItem, addModuleItem,
        migrateAllToFirebase, syncAllModules: sincronizarTodosModulos,
        ...
        carregarModulo: async (nomeModulo) => {
            ...
            if (isOnline && db) {
                try {
                    ...
                    if (data && data.length > 0) { safeSetItem(nomeModulo, data); return data; }
                } catch(e) {}
            }
